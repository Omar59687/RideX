...
select throws_ok($$select public.backend_record_payment_attempt((select id from public.payments where method='card'),'initialAuthorization',2500,'auth-3','provider')$$,'55000','Initial authorization requires an accepted pending Card Payment.','additional initial authorization is rejected after Card authorization');
