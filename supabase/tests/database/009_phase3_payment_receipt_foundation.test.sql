...
select lives_ok($$select public.backend_transition_payment((select id from public.payments where method='card'),1,null,'cardPaymentAuthorized')$$,'live replacement authorization authorizes Card payment');
...
insert into public.payment_attempts (payment_id,type,status,requested_amount_fils,currency,idempotency_key,completed_at) values ((select id from public.payments where method='card'),'providerStatusVerification','succeeded',2500,'JOD','verified-capture',now());
...
insert into public.payment_attempts (payment_id,type,status,requested_amount_fils,currency,idempotency_key,completed_at) values ((select id from public.payments where method='card'),'capture','succeeded',2500,'JOD','capture-success',now());
...
select lives_ok($$select public.admin_request_refund((select id from public.payments where method='card'),3,'duplicate_charge')$$,'Admin requests a full Card refund');
select is((select amount_fils from public.refunds),2500,'refund is full payment value');
select is((select card_status::text from public.payments where method='card'),'refundPending','refund request retains pending status');
select lives_ok($$select public.admin_request_refund((select id from public.payments where method='card'),4,'duplicate_charge')$$,'active refund request is idempotent');
reset role;
...
select is((select count(*) from public.payments),2::bigint,'Rider reads own payments');
select is((select count(*) from public.receipts),1::bigint,'Rider reads own receipt');
...
select is((select count(*) from public.payments),0::bigint,'Driver cannot read payments');
select is((select count(*) from public.receipts),1::bigint,'assigned Driver reads restricted receipt');
reset role;
...
select is((select count(*) from public.payments),0::bigint,'unrelated Rider cannot read payments');
select is((select count(*) from public.receipts),0::bigint,'unrelated Rider cannot read receipts');
reset role;
